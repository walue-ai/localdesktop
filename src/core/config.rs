...
fn default_check() -> String {
    "pacman -Q weston".to_string()
}

fn default_install() -> String {
    "stdbuf -oL pacman -Syu weston xterm kcalc noto-fonts noto-fonts-cjk noto-fonts-emoji --noconfirm --noprogressbar".to_string()
}

fn default_launch() -> String {
    "WAYLAND_DISPLAY=wayland-0 XDG_RUNTIME_DIR=/tmp QT_SCALE_FACTOR=0.7 QT_AUTO_SCREEN_SCALE_FACTOR=0 QT_FONT_DPI=72 QT_WAYLAND_FORCE_DPI=72 QT_ENABLE_HIGHDPI_SCALING=0 QT_SCREEN_SCALE_FACTORS=0.7 GDK_SCALE=0.7 GDK_DPI_SCALE=0.7 FONTCONFIG_PATH=/tmp/fontconfig FREETYPE_PROPERTIES=truetype:interpreter-version=40 kcalc".to_string()
}
